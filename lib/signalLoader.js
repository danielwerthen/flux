...
Loader.prototype._onUpdate = function (file, data) {
	...
	self.signals[file].stop();
	var sig = self.signals[file] = self.createSignal(file);
	sig.load(data);
	self.signatures[sig.signature] = sig;
	if (self.options.autostart) sig.start();
	self.emit('loaded', sig);
};
Loader.prototype._onRemove = function (file) {
	...
	if (self.signals[file]) {
		...
		self.signatures[self.signals[file].signature] = undefined;
...
Loader.prototype.get = function (signature) {
