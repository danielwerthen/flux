...
Executor.prototype = {
	...
	call: function (signal, name, args, callback) {
		if (!this.funcs[name])
			throw new Error('No function were found with the name: ' + name);
		this.funcs[name].func.apply(signal, _.flatten([ args, callback ], true));
	}
};
