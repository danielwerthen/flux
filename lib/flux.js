...
var Flux = module.exports = function (options) {
	...
	this.loader = new Loader(function (name) {
		return new Signal(self.localMap, self.remoteMap, name);
	}, this.options);
};
...
Flux.prototype = {
	...
	duplex: function (options) {
		var self = this
			, sim = httpPipe.listen(options, _.bind(self._handleData, self))
			, dup = duplex.listen(options, _.bind(self._handleData, self)
				, _.bind(self._handleDuplexPipe, self));
		return function (req, res, next) {
			sim(req, res, function () {
				dup(req, res, next);
			});
		};
	},
